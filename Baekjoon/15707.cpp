...
int main(void){
	std::string a, b, r;
	std::cin >> a >> b >> r;
	BigInteger b_a = BigInteger(a);
	BigInteger b_b = BigInteger(b);
	BigInteger b_r = BigInteger(r);
	BigInteger result = b_a * b_b;

	if (result > b_r) std::cout << "overflow";
	else std::cout << std::string(result);
